	...
	die('Images Could not be resized');
        ...
        die( "Sorry, there was an error uploading your file.");
            ...
            if($_SESSION['SW_U_ACCESS'] < 2){
			 ...
			 $inssql = "INSERT INTO `sw_products_list`(`pr_code`,`pr_img`,`pr_img_2`,`pr_img_3`,`pr_name`, `pr_type`, `pr_desc`, `pr_price`,`pr_qty`) VALUES (
            '".$_POST['pr_code']."',
				'".$target_file."',
					'".$target_file_2."',
						'".$target_file_3."',
...
if(count($error_dump) == 0){
	...
	header('Location: edit_order.php?or_edit=h5346h&edihchk='.$redir_hash);
}
...
if(isset($_POST['or_pr_del'])){
	if(isset($_POST['pr_or_delid']) && ctype_alnum($_POST['pr_or_delid'])){
		...
		if($conn->query($del_or_pr_sql)){
			...
			$getorid_s = "select ih.or_id from sw_order_items ihy left join sw_orders ih on
