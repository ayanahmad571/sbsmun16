...
SELECT sh_pr_id_rel_pr_id FROM `sw_products_list_show` where sh_valid =1 ";
 			...
 			if(count($nopr) >1){
				$prsql = "
SELECT pr_name,pr_id,pr_code FROM `sw_products_list` where pr_valid =1 and not(pr_id = ".implode(' or pr_id =',$nopr).") ";
			}else if(count($nopr) == 1){
				$prsql = "
SELECT pr_name,pr_id,pr_code FROM `sw_products_list` where pr_valid =1 and not(pr_id = ".$nopr[0].")  ";
			}else{
				$prsql = "
SELECT pr_name,pr_id,pr_code FROM `sw_products_list` where pr_valid =1  ";
			}
...
<div class="form-group ">
 ...
 <select class="form-control " name="pr_sh_id">
        <?php
