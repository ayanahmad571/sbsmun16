...
foreach($_POST as $key=>$v){
	...
	if(!is_array($_POST[$key])){
		if($key == 'edit_pg_btxt' or $key == 'pg_banner' or $key == 'pg_img_src' or $key=='ads_edit_code' or $key==
		'adrs_desc' or $key=='com_edit_desc' or $key == 'com_desc' or $key=='nl_edit_desc'){
			$_POST[$key] = str_replace('\\r\\n','',trim($conn->escape_string($v)));
		}else{
	...
	echo '<!-- brand -->
            <div class="logo">
                <a href="#" class="logo-expanded">
                    <i class=" ion-home"></i>
                    <span class="nav-label">SBSMUN</span>
                </a>
            </div>
            <!-- / brand -->';
}
...
####
			  ...
			  if(trim($_SESSION['ADM_U_TABS']) == 'all'){
 $sql = "SELECT * FROM `adm_modules` WHERE `mo_min_level`<=".$_SESSION['ADM_U_ACCESS']." and mo_valid =1
			
			";
			
			}else{
				
$sql = "SELECT * FROM `adm_modules` WHERE `mo_min_level`<=".$_SESSION['ADM_U_ACCESS']." and mo_valid =1 and mo_id in (".$_SESSION['ADM_U_TABS'].")
			";
			}
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		if($row['mo_sub_mod'] == 1){
			...
			$actcswl = "SELECT * FROM `adm_submod` WHERE `sub_mo_rel_mo_id`= ".$row['mo_id']." and sub_valid=1 and sub_href='".trim(basename($_SERVER['PHP_SELF']))."'";
...
if ($act_res->num_rows == 1) {
	...
	echo '<ul class="list-unstyled">
			...
			$submod = "SELECT * FROM `adm_submod` where sub_valid = 1 and sub_mo_rel_mo_id='".$row['mo_id']."'";
...
<?php
        ...
        <title>SBS MUN - Admin Dashboard </title>
...
function get_stu_pics($stu_id,$sess_id){
	if(!is_numeric($stu_id.$sess_id)){
		die('BAD');
	}
	echo 'http://stepsys.org/files/images/stu_pics/'.$stu_id.'_'.$sess_id.'.jpg' ;
}
?>
