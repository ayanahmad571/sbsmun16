...
$target_file = '../sbsmun/uploads/'.$fl_name;
$todb= 'uploads/'.$fl_name;
$uploadOk = 1;
$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
// Check if image file is a actual image or fake image
if(isset($_POST["upld_rnd_fle"])) {
...
$uploadOk = 1;
	...
	for($rr =0;$rr< $rp;$rr++){
	##
	$mat = uniqid().'_'.md5(sha1(md5(sha1(time().microtime())))).basename($_FILES["trip"]["name"][$rr]);
